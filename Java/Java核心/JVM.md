...
![](../../picture/JVM%E7%B1%BB%E5%8A%A0%E8%BD%BD%E5%99%A8.png)

* 启动类加载器（根加载器）是用C/C++编写的，主要负责加载jre\lib目录下的类
* 扩展类加载器主要负责加载jre\lib\ext目录下的类
* 应用程序加载器主要负责加载应用程序的主函数类
...
存储静态变量、常量、类信息（构造方法、接口定义）、运行时常量池，实例变量存在堆中与方法区无关
